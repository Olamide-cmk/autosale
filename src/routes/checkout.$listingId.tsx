  ...
  async function handlePay(e: React.FormEvent) {
    ...
    try {
      ...
      if (err instanceof Error && err.message.includes("ALREADY_SOLD")) {
        setPayError("Cette voiture vient d'être vendue à un autre acheteur.");
        setAlreadySold(true);
      } else {
        setPayError("Le paiement a échoué. Merci de réessayer.");
      }
    } finally {
      setPaying(false);
    }
    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <main id="main-content" className="container-page flex flex-col items-center justify-center py-24 text-center">
          <h1 className="text-3xl font-bold">Cette voiture n'est plus disponible</h1>
          <p className="mt-2 max-w-md text-muted-foreground">Elle a déjà été vendue à un autre acheteur.</p>
          <Link
            to="/"
    ...
    <div className="min-h-screen bg-background">
      ...
      <main id="main-content" className="container-page max-w-2xl py-12">
        <div className="mb-6 flex items-center gap-2 rounded-lg border border-accent/40 bg-accent/10 px-4 py-3 text-sm text-foreground">
          ...
          <p>
            <strong>Mode démonstration.</strong> Aucun paiement réel n'est traité ici — aucune carte
            n'est débitée, aucun numéro de carte n'est transmis à un vrai processeur de paiement. Ce
            flux illustre uniquement l'expérience d'achat par carte bancaire.
          </p>
        </div>
        ...
        {step !== "done" ? (
            ...
            <div className="h-fit rounded-xl border bg-card p-5">
              ...
              <dl className="mt-4 space-y-2 text-sm">
                <div className="flex justify-between">
                  <dt className="text-muted-foreground">Prix demandé</dt>
            ...
            {step === "review" ? (
              <div className="rounded-xl border bg-card p-5">
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  Vendeur : {listing.sellerName} · {listing.location}
                </p>
                <p className="mt-1 text-sm text-muted-foreground">
                  La remise du véhicule est à organiser séparément avec le vendeur une fois le
                  paiement confirmé.
                </p>
                <button
                  type="button"
              ...
              <form onSubmit={handlePay} className="rounded-xl border bg-card p-5">
                <h2 className="flex items-center gap-2 text-lg font-semibold">
                  <Lock className="size-4" /> Paiement par carte bancaire (démo)
                </h2>
                <div className="mt-4 space-y-3">
                  ...
                  <div>
                    <label className="eyebrow block">Numéro de carte</label>
                    <Input
                      className="mt-1"
                      inputMode="numeric"
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="eyebrow block">Expiration</label>
                      <Input
                        className="mt-1"
                    ...
                    <div>
                      <label className="eyebrow block">CVC</label>
                      <Input
                        className="mt-1"
                        inputMode="numeric"
                        value={form.cardCvc}
                ...
                >
                  {paying ? "Paiement en cours…" : `Payer ${formatUsd(total)} (démo)`}
                </button>
              </form>
            )}
          </div>
          ...
          <div className="mt-10 flex flex-col items-center rounded-xl border bg-card p-12 text-center">
            ...
            <h2 className="mt-6 text-2xl font-bold">Paiement simulé avec succès</h2>
            <p className="mt-2 max-w-md text-muted-foreground">
              Carte terminant par {cardLast4} débitée (démo) de {formatUsd(total)}. Dans un vrai
              déploiement, le vendeur serait notifié pour organiser la remise de votre{" "}
